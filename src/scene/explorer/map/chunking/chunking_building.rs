...
fn spawn_chunk(
    ...
    let mut tile_storage = TileStorage::empty(chunks.building.xy().into());
        ...
        .insert((
            TilemapBundle {
                ...
                render_settings: TilemapRenderSettings {
                    render_chunk_size: chunks.building.xy() * 2,
                    ..Default::default()
                },
                ..Default::default()
            },
        ...
        for y in (camera_chunk_pos.y - 2)..(camera_chunk_pos.y + 2) {
            for x in (camera_chunk_pos.x - 2)..(camera_chunk_pos.x + 2) {
                if !chunk_manager.spawned_chunks.contains(&IVec2::new(x, y)) {
                    let ulam_val = ulam::get_value_from_xy(x, y);
                    let check = world_map.map.get(&ulam_val);
                    if let Some(s) = check {
